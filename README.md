...
- **更新 DNS 记录**：访问 `https://your-worker-domain.workers.dev/update?name=your_dns_record_name&key=your_access_key&ip=1.2.3.4,2.2.2.2`
  - `name`：需要更新的 DNS 记录名称，使用完整域名（如 `my.example.com`）
  - `key`：访问密钥，用于验证更新请求，要等于worker中的my.example.com__access_key值
  - `ip`：要更新的公网 IP，多个 IP 用逗号分隔（可选；不提供时使用请求客户端 IP），例如 `1.1.1.1,2.2.2.2`
